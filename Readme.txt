Steps to execute:
1.Run the controller file from the command ---- ryu-manager [filepath]/Arp_mitigate_controller.py
2.Create the mininet topology with command ---- sudo mn --topo single,4 --mac --controller=remote --switch ovsk,protocols=OpenFlow13
3.Try "pingall" command to check if all links are connected.
4.Open each host terminal. Assign host 1 as DHCP server and get the ip addresses of other hosts using below commands:
...
From host 2 call the Arp_attack.py using following command ---- python3 [filepath]/arp_attack.py.
